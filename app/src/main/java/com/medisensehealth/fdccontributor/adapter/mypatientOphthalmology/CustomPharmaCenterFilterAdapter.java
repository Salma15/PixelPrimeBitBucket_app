...
public class CustomPharmaCenterFilterAdapter extends ArrayAdapter<PharmaCentreList> {
    Context mContext;
    private List<PharmaCentreList> animalNamesList = null;
    private ArrayList<PharmaCentreList> arraylist;
    LayoutInflater inflater;

    public CustomPharmaCenterFilterAdapter(Context context, int resourceId,
                                               List<PharmaCentreList> items) {
        super(context, resourceId, items);
        this.mContext = context;
        this.animalNamesList = items;
        inflater = LayoutInflater.from(mContext);
        this.arraylist = new ArrayList<PharmaCentreList>();
    ...
    public View getView(final int position, View convertView, ViewGroup parent)    {
        ...
        PharmaCentreList rowItem = getItem(position);
        ...
        holder.m_title.setText(rowItem.getPharmaName());

        String test = rowItem.getPharmaName().trim();
    ...
    public void filter(String charText) {
        ...
        if (charText.length() == 0) {
            ...
            for (PharmaCentreList wp : arraylist) {
                if (wp.getPharmaName().toLowerCase(Locale.getDefault()).contains(charText)) {
