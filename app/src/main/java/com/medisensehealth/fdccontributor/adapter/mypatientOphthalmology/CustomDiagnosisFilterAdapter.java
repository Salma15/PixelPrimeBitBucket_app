...
public class CustomDiagnosisFilterAdapter extends ArrayAdapter<Diagnosis> {

    Context mContext;
    private List<Diagnosis> animalNamesList = null;
    private ArrayList<Diagnosis> arraylist;
    LayoutInflater inflater;

    public CustomDiagnosisFilterAdapter(Context context, int resourceId,
    ...
    public View getView(final int position, View convertView, ViewGroup parent)    {
        ...
        Diagnosis rowItem = getItem(position);
        ...
        holder.m_title.setText(rowItem.getICDName());

        String test = rowItem.getICDName().trim();
    ...
    public void filter(String charText) {
        ...
        if (charText.length() == 0) {
            ...
            for (Diagnosis wp : arraylist) {
                if (wp.getICDName().toLowerCase(Locale.getDefault()).contains(charText)) {
